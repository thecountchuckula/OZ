package tcc.OZ.lib;

import net.minecraft.block.Block;
import net.minecraft.item.Item;
import net.minecraft.item.ItemStack;
import tcc.OZ.armor.ModArmor;
import tcc.OZ.blocks.ModBlocks;
import tcc.OZ.items.ModItems;

public class ItemStacks {

    // OZ Armor
    public static final ItemStack dresstop = new ItemStack(ModArmor.dressTop, 1, 0);
    public static final ItemStack dressbottom = new ItemStack(ModArmor.dressBottom, 1, 0);
    public static final ItemStack silverslippers = new ItemStack(ModArmor.silverSlippers, 1, 0);
    
    // Vanilla Items
    public static final ItemStack iron = new ItemStack(Item.ingotIron, 1, 0);
    
    // Vanilla Wool
    public static final ItemStack whitewool = new ItemStack(Block.cloth, 1, 0);
    public static final ItemStack lightbluewool = new ItemStack(Block.cloth, 1, 3);
    
    // Vanilla Stained Clay
    public static final ItemStack whitestainedclay = new ItemStack(Block.stainedClay, 1, 0);
    public static final ItemStack orangestainedclay = new ItemStack(Block.stainedClay, 1, 1);
    public static final ItemStack magentastainedclay = new ItemStack(Block.stainedClay, 1, 2);
    public static final ItemStack lightbluestainedclay = new ItemStack(Block.stainedClay, 1, 3);
    public static final ItemStack yellowstainedclay = new ItemStack(Block.stainedClay, 1, 4);
    public static final ItemStack limestainedclay = new ItemStack(Block.stainedClay, 1, 5);
    public static final ItemStack pinkstainedclay = new ItemStack(Block.stainedClay, 1, 6);
    public static final ItemStack graystainedclay = new ItemStack(Block.stainedClay, 1, 7);
    public static final ItemStack lightgraystainedclay = new ItemStack(Block.stainedClay, 1, 8);
    public static final ItemStack cyanstainedclay = new ItemStack(Block.stainedClay, 1, 9);
    public static final ItemStack purplestainedclay = new ItemStack(Block.stainedClay, 1, 10);
    public static final ItemStack bluestainedclay = new ItemStack(Block.stainedClay, 1, 11);
    public static final ItemStack brownstainedclay = new ItemStack(Block.stainedClay, 1, 12);
    public static final ItemStack greenstainedclay = new ItemStack(Block.stainedClay, 1, 13);
    public static final ItemStack redstainedclay = new ItemStack(Block.stainedClay, 1, 14);
    public static final ItemStack blackstainedclay = new ItemStack(Block.stainedClay, 1, 15);
    
    //OZ Blocks
    public static final ItemStack emeraldsmooth = new ItemStack(ModBlocks.EmeraldSmooth, 1, 0);
    public static final ItemStack emeraldstone = new ItemStack(ModBlocks.EmeraldStone, 1, 0);
    public static final ItemStack emeraldbrick = new ItemStack(ModBlocks.EmeraldBrick, 1, 0);
    public static final ItemStack emeraldcarved = new ItemStack(ModBlocks.EmeraldCarved, 1, 0);
    //public static final ItemStack door = new ItemStack(ModItems.EmeraldDoor, 1);

}
